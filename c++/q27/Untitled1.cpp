...
bool test_add_binaries_signed(); // which returns true if the student’s implementation of add_binaries function
// is correct; false otherwise. Student shall not modify this function
...
int binary_to_decimal_signed(string s)
{
    ...
    int dec = 0;
    if(signBit == '1'){
    	dec = binary_to_decimal(twos_complement(s));
    	dec *= -1;
	}else{
...
{ 
	...
	if(binary[0] == '1'){
		...
		if(isNegative){
			...
			binary = signed_extension(binary);
	}else{
		binary = signed_extension(binary);
	}
...
string add_binaries_signed(string b1, string b2)
{
    ...
    int res = binary_to_decimal_signed(b1) + binary_to_decimal_signed(b2);
    string binary = decimal_to_binary_signed(res);
    return binary;
}
