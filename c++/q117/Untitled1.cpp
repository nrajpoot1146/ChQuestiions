...
int main( )
{
	...
	while(i<7){
		cout<<"Enter a number: ";
		cin>>num; // read number from Binary heap
		if (i==0){
			bh.insert(num);
		} else {
			int p = ((i - 1) / 2 ) + 1; // get parent of current element
			if (bh.getith(p) < num){ // check whether binary heap condition is violate or not
				cout<<num<<" violated the heap. Bye..."; // if yes print violation message
				break;
			}
			bh.insert(num); // if no insert num to binary heap
		}
		i++;
	}
