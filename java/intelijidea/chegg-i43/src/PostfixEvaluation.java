public class PostfixEvaluation {
    ...
    public static int postfixEval(String exp) {
        ...
        for (int i = 0; i < symbols.length; i++) {
            ...
            if (isOperand(symbol)){
                // if symbol is operand, push it into stack
                stack.push(getOperand(symbol));
            }else if(isOperator(symbol)){
                // if symbol is operator, pop first two operand from stack
                int op1 = stack.pop();
                int op2 = stack.pop();

                int operator = getOperator(symbol); // get operator
                int res = 0;
                if (operator == ADD_OP){ // if operator is addition operator
                    res = op1 + op2;
                }else if(operator == SUB_OP){ // if operator is subtraction operator
                    res = op2 - op1;
                }else if(operator == MUL_OP){ // if operator is multiplicative operator
                    res = op1 * op2;
                }else{ // if operator is division operator
                    res = op2 / op1;
                }
                stack.push(res);
            }
